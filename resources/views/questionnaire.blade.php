...
<html>
	<head>
		...
		<div id="dataForm" class="formWrap">
            ...
            <p class="radioTitle formRadio" title="{{$item->title}}"><span class="blackFont">{{$item->title}}</span>(单选<span class="redFont">*</span>)</p>
		...
		<script type="text/javascript" charset="utf-8">
			...
			mui('#dataForm').on('click','.submitbtn',function(){
				...
				mui.each(radioData,function(index, item){
					let answerRadio = mui('#dataForm .answerRadio'+index+':checked');
					if(answerRadio.length <= 0 ){
						mui.alert('请填写'+item['title'])
						isOk = false;
						return false;
					}
					radioData[index]['answre'] = answerRadio[0].value;
				})
